...
public class EnrichmentNERServiceImpl {
    ...
    /*
     * Loading Solr service for finding the positions of Entities in the original
     * text
     */
    ...
    public List<NamedEntityImpl> getEntities(String storyId, String itemId, String property, List<String> nerTools)
            throws Exception {
        return persistentNamedEntityService.findNamedEntitiesWithAdditionalInformation(storyId, itemId, property,
                nerTools, false);
    }

    /**
     * This function should pnly be called when updating the named entities of a
     * single story. To compute the named entities for all stories use the function
     * updatedNamedEntitiesForText, followed by the addLinkingInformation and
     * computePreferredWikidataIds.
     * 
     * @param storyId
     * @param type
     * @param nerTools
     ...
     * @throws Exception
     */
    public List<NamedEntityImpl> createNamedEntitiesForStory(String storyId, String property, List<String> nerTools,
            List<String> linking, String translationTool, boolean updateStory) throws Exception {
        ...
        persistentNamedEntityAnnotationService.deleteNamedEntityAnnotation(storyId, null, property,
        ...
        if (StringUtils.isBlank(textAndLanguage.get(EnrichmentConstants.FIELD_TEXT))
                || StringUtils.isBlank(textAndLanguage.get(EnrichmentConstants.FIELD_LANG))) {
            return result;
        }
        ...
        // sometimes some fields for NER can be empty for items which causes problems in
        // the method applyNERTools
        Set<ObjectId> namedEntitiesToUpdateLinking = updatedNamedEntitiesForText(nerTools, textForNer, languageForNer,
                property, storyId, null, linking, true);
    ...
    /**
     * This function should only be called when updating the named entities of a
     * single story. To compute the named entities for all stories use the function
     * updatedNamedEntitiesForText, followed by the addLinkingInformation and
     * computePreferredWikidataIds.
     * 
     ...
     * @throws Exception
     */
    public List<NamedEntityImpl> createNamedEntitiesForItem(ItemEntityImpl item, String property,
            List<String> nerTools, List<String> linking, String translationTool, boolean updateItem) throws Exception {
        List<NamedEntityImpl> result = new ArrayList<>();
        ...
        persistentNamedEntityAnnotationService.deleteNamedEntityAnnotation(item.getStoryId(), item.getItemId(), property,
                EnrichmentConstants.MONGO_SKIP_FIELD);

        Map<String, String> textAndLanguage = getItemTextForNER(item, translationTool, property, updateItem);
        if (StringUtils.isBlank(textAndLanguage.get(EnrichmentConstants.FIELD_TEXT))
                || StringUtils.isBlank(textAndLanguage.get(EnrichmentConstants.FIELD_LANG))) {
            return result;
        }
        ...
        // the method applyNERTools
        Set<ObjectId> namedEntitiesToUpdateLinking = updatedNamedEntitiesForText(nerTools, textForNer, languageForNer,
                property, item.getStoryId(), item.getItemId(), linking, true);
        // add linking to the named entities
        for (ObjectId neId : namedEntitiesToUpdateLinking) {
            NamedEntityImpl ne = persistentNamedEntityService.findNamedEntity(neId);
            result.add(ne);
    ...
    /**
     * When this function is called for multiple ner tools one after another, make
     * sure it is first called with the nerTool="DBpedia_Spotlight". For each ner
     * tool the analysis is done separately because different tools may find the
     * same entities on different positions in the text and we would like to
     * separate those results, otherwise all positions of the entities would be in
     * the same list and it cannot be clear which positions belong to which ner tool
     * analyser.
     * 
     * @param nerTool
     * @param textForNer
     ...
     * @throws Exception
     */
    public Set<ObjectId> updatedNamedEntitiesForText(List<String> nerTools, String textForNer, String languageForNer,
            String fieldType, String storyId, String itemId, List<String> linking, boolean matchType) throws Exception {
        ...
        for (String nerTool : nerToolsReordered) {
            TreeMap<String, List<NamedEntityImpl>> tmpResult = applyNERTools(nerTool, textForNer, languageForNer,
                    fieldType, storyId, itemId);
            if (tmpResult == null) {
                continue;
            }
            for (String classificationType : tmpResult.keySet()) {

                if (isRestrictedClassificationType(classificationType))
                    continue;
                ...
                for (NamedEntityImpl tmpNamedEntity : tmpResult.get(classificationType)) {
                    ...
                    if (tmpNamedEntity.getType().equalsIgnoreCase(NERClassification.AGENT.toString())
                            && HelperFunctions.toArray(tmpNamedEntity.getLabel(), null).length < 2) {
                        continue;
                    }

                    NamedEntityImpl dbNamedEntity = persistentNamedEntityService.findEqualNamedEntity(tmpNamedEntity);
                    saveNamedEntityAndPositions(tmpNamedEntity, dbNamedEntity, nerTool, linking,
                            namedEntityObjectsToRecomputeLinking);
                }
            }
        }
    ...
    private void saveNamedEntityAndPositions(NamedEntityImpl newNamedEntity, NamedEntityImpl existingNamedEntity,
            String nerTool, List<String> linking, Set<ObjectId> namedEntityObjectsToRecomputeLinking) throws Exception {
        if (existingNamedEntity != null) {
            boolean foundMatchingPosition = false;
            // update existing position entity if it exists
            List<PositionEntityImpl> existingPosition = persistentPositionEntityService.findPositionEntities(
                    existingNamedEntity.get_id(), newNamedEntity.getPositionEntity().getStoryId(),
            ...
            if (!existingPosition.isEmpty()) {
                ...
                for (Map.Entry<Integer, String> newOffset : newNamedEntity.getPositionEntity()
                        .getOffsetsTranslatedText().entrySet()) {
                    ...
                    if (existingPosition.get(0).getOffsetsTranslatedText().containsKey(newOffsetKey)) {
                        String existingOffsetNerTools = existingPosition.get(0).getOffsetsTranslatedText()
                                .get(newOffsetKey);
                        if (!existingOffsetNerTools.contains(newOffsetValue)) {
                            existingPosition.get(0).getOffsetsTranslatedText().put(newOffsetKey,
                                    existingOffsetNerTools + "," + newOffsetValue);
            ...
            // if no existing position is found, add the position entity as new to the
            // existing named entity
    ...
    public Map<String, String> getStoryTextForNER(String storyId, String translationTool, String type,
            boolean updateStoryBool) throws Exception {
        ...
        if (updateStoryBool) {
            //TODO: switch to delete translations
            updatedStory = enrichmentStoryAndItemStorageService.updateStoryFromTranscribathon(storyId, fiedlsToUpdate, true);
        } else {
    ...
    public Map<String, String> getItemTextForNER(ItemEntityImpl item, String translationTool, String property,
            boolean updateItemBool) throws Exception {
        Map<String, String> results = new HashMap<>();
...
//        }
        ...
        String translatedText = enrichmentTranslationService.translateItem(item, property, translationTool,
                true);
        if (!StringUtils.isBlank(translatedText)) {
            results.put(EnrichmentConstants.FIELD_TEXT, translatedText);
            results.put(EnrichmentConstants.FIELD_LANG, EnrichmentConstants.defaultTargetTranslationLang2Letter);
    ...
    private TreeMap<String, List<NamedEntityImpl>> applyNERTools(String nerTool, String text, String language,
            String fieldUsedForNER, String storyId, String itemId) throws Exception {
    ...
    private void updateNamedEntityPositionEntity(TreeMap<String, List<NamedEntityImpl>> mapCurrentResult,
        ...
        if (mapCurrentResult == null || mapCurrentResult.isEmpty())
            return;
        ...
        for (Map.Entry<String, List<NamedEntityImpl>> categoryList : mapCurrentResult.entrySet()) {
            for (NamedEntityImpl entity : categoryList.getValue()) {
                if (entity.getPositionEntity() == null)
                    continue;
    ...
    public NamedEntityAnnotationCollection getAnnotations(String storyId, String itemId, String property)
            throws Exception {
        List<NamedEntityAnnotationImpl> entities = persistentNamedEntityAnnotationService.findAnnotations(storyId,
                itemId, property, EnrichmentConstants.MONGO_SKIP_FIELD, EnrichmentConstants.MONGO_SKIP_LIST_FIELD);
        return new NamedEntityAnnotationCollection(configuration.getAnnotationsIdBaseUrl(),
                configuration.getAnnotationsTargetStoriesBaseUrl(), configuration.getAnnotationsTargetItemsBaseUrl(),
                configuration.getAnnotationsCreator(), entities, storyId, itemId);
    }

    public NamedEntityAnnotationCollection createAnnotationsFullWorkflow(String storyId, String itemId, String property,
            List<String> storyFieldsToUpdateOtherEntities) throws Exception {
        //TODO: separate item enrichment workflow from story enrichment workflow
        ItemEntityImpl updatedItem = null;
        StoryEntityImpl updatedStory = null;
        if (itemId != null) {
            updatedItem = enrichmentStoryAndItemStorageService.updateItemFromTranscribathon(storyId,
                    itemId, true);
        } else {
            updatedStory = enrichmentStoryAndItemStorageService.updateStoryFromTranscribathon(storyId,
                    storyFieldsToUpdateOtherEntities, true);
        }
        ...
        if (existingAnnos != null && existingAnnos.getItems().size() > 0) {
            ...
            // if there are no named and/or position entities in the db perform the NER
            // analysis
            List<PositionEntityImpl> positionEntities = persistentPositionEntityService
                    .findPositionEntities(EnrichmentConstants.MONGO_SKIP_OBJECT_ID_FIELD, storyId, itemId, property);
            if (positionEntities.isEmpty()) {
                String nerTools = NerTools.Dbpedia.getStringValue() + "," + NerTools.Stanford.getStringValue();
                List<String> linkingList = new ArrayList<>(
                        Arrays.asList(HelperFunctions.toArray(EnrichmentConstants.WIKIDATA_LINKING, ",")));
                List<String> nerToolsList = new ArrayList<>(Arrays.asList(HelperFunctions.toArray(nerTools, ",")));
                if (updatedItem != null) {
                    createNamedEntitiesForItem(updatedItem, property, nerToolsList, linkingList,
                            EnrichmentConstants.defaultTranslationTool, false);
                } else {
                    createNamedEntitiesForStory(storyId, property, nerToolsList, linkingList,
                            EnrichmentConstants.defaultTranslationTool, false);
                }
            }
    ...
    public NamedEntityAnnotationCollection createAnnotationsForStoryOrItem(String storyId, String itemId,
        ...
        return new NamedEntityAnnotationCollection(configuration.getAnnotationsIdBaseUrl(),
                configuration.getAnnotationsTargetStoriesBaseUrl(), configuration.getAnnotationsTargetItemsBaseUrl(),
                configuration.getAnnotationsCreator(), namedEntityAnnos, storyId, itemId);
    }
    ...
    public NamedEntityAnnotationImpl createAndSaveAnnotationsForPosition(PositionEntityImpl pe) throws Exception {
        ...
        if (preferredWikiId != null) {
            ...
            String wikidataJSONLocal = HelperFunctions
                    .getWikidataJsonFromLocalFileCache(configuration.getEnrichWikidataDirectory(), preferredWikiId);
            ...
            if (StringUtils.isBlank(wikidataJSON)) {
                ...
                HelperFunctions.saveWikidataJsonToLocalFileCache(configuration.getEnrichWikidataDirectory(),
                        preferredWikiId, wikidataJSON);
            }
            ...
            jsonElement = wikidataService.getJSONFieldFromWikidataJSON(wikidataJSON,
                    EnrichmentConstants.PREFLABEL_JSONPROP);
            ...
            // body description
            jsonElement = wikidataService.getJSONFieldFromWikidataJSON(wikidataJSON,
                    EnrichmentConstants.DESCRIPTION_JSONPROP);
            if (!jsonElement.isEmpty()) {
            ...
            if (ne.getType().equalsIgnoreCase(NERClassification.AGENT.toString())) {
                ...
                jsonElement = wikidataService.getJSONFieldFromWikidataJSON(wikidataJSON,
                        EnrichmentConstants.GIVEN_NAME_JSONPROP);
                if (!jsonElement.isEmpty()) {
                    // for each collected name in the form of wikidata Q-identifier, fetch the
                    // english label for that wikidata id
                    for (int i = 0; i < jsonElement.size(); i++) {
                        String givenNameWikiId = EnrichmentConstants.WIKIDATA_ENTITY_BASE_URL
                                + jsonElement.get(i).get(0);
                        String givenNameWikiJson = wikidataService.getWikidataJSONFromRemote(givenNameWikiId);
                        List<List<String>> givenNameJsonElement = wikidataService
                                .getJSONFieldFromWikidataJSON(givenNameWikiJson, "labels.en.value");
                ...
                jsonElement = wikidataService.getJSONFieldFromWikidataJSON(wikidataJSON,
                        EnrichmentConstants.FAMILY_NAME_JSONPROP);
                if (!jsonElement.isEmpty()) {
                    // for each collected family name in the form of wikidata Q-identifier, fetch
                    // the english label for that wikidata id
                    for (int i = 0; i < jsonElement.size(); i++) {
                        String familyNameWikiId = EnrichmentConstants.WIKIDATA_ENTITY_BASE_URL
                                + jsonElement.get(i).get(0);
                        String familyNameWikiJson = wikidataService.getWikidataJSONFromRemote(familyNameWikiId);
                        List<List<String>> familyNameJsonElement = wikidataService
                                .getJSONFieldFromWikidataJSON(familyNameWikiJson, "labels.en.value");
                ...
                jsonElement = wikidataService.getJSONFieldFromWikidataJSON(wikidataJSON,
                        EnrichmentConstants.PROFESSIONOROCCUPATION_JSONPROP);
                if (!jsonElement.isEmpty()) {
                    for (int i = 0; i < jsonElement.size(); i++) {
                        String occupationWikiId = EnrichmentConstants.WIKIDATA_ENTITY_BASE_URL
                                + jsonElement.get(i).get(0);
                        String occupationWikiJson = wikidataService.getWikidataJSONFromRemote(occupationWikiId);
                        List<List<String>> occupationJsonElement = wikidataService
                                .getJSONFieldFromWikidataJSON(occupationWikiJson, "labels.en.value");
                ...
                jsonElement = wikidataService.getJSONFieldFromWikidataJSON(wikidataJSON,
                        EnrichmentConstants.LATITUDE_JSONPROP);
                if (!jsonElement.isEmpty()) {
                    body_lat = Float.valueOf(jsonElement.get(0).get(0));
                }

                jsonElement = wikidataService.getJSONFieldFromWikidataJSON(wikidataJSON,
                        EnrichmentConstants.LONGITUDE_JSONPROP);
                if (!jsonElement.isEmpty()) {
            ...
            NamedEntityAnnotationImpl newAnno = new NamedEntityAnnotationImpl(configuration.getAnnotationsIdBaseUrl(),
                    ...
                    ne.getLabel(), entityPrefLabel, pe.getFieldUsedForNER(), ne.getType(), score, foundByNer,
                    linkedByNer, body_description, body_givenName, body_familyName, body_professionOrOccupation,
                    body_lat, body_long);
    ...
    @Async
    public CompletableFuture<Boolean> createAndSaveAnnotationsForPosition_parallel(PositionEntityImpl pe)
            throws Exception {
        List<String> foundByNer = new ArrayList<>();
        List<String> linkedByNer = new ArrayList<>();
        NamedEntityImpl ne = persistentNamedEntityService.findNamedEntity(pe.getNamedEntityId());
    ...
    public String choosePreferredWikiId(PositionEntityImpl pe, NamedEntityImpl ne, List<String> foundByNer,
            List<String> linkedByNer) {
        ...
        Optional<Integer> optNerToolsBothFound = pe.getOffsetsTranslatedText().entrySet().stream()
                .filter(e -> (e.getValue().contains(NerTools.Stanford.getStringValue())
                        && e.getValue().contains(NerTools.Dbpedia.getStringValue())))
        ...
        Optional<Integer> optNerToolsDbpedia = pe.getOffsetsTranslatedText().entrySet().stream()
                .filter(e -> e.getValue().contains(NerTools.Dbpedia.getStringValue())).map(Map.Entry::getKey)
                .findFirst();
        ...
        Optional<Integer> optNerToolsStanford = pe.getOffsetsTranslatedText().entrySet().stream()
                .filter(e -> e.getValue().contains(NerTools.Stanford.getStringValue())).map(Map.Entry::getKey)
                .findFirst();

        /*
         * Here we check if the given named entity is found by stanford, dbpedia or
         * both, and if it is linked by stanford, dbpedia, or both. Based on that the
         * annotations are generated, by assigning the proper score.
         */
        ...
        if (optNerToolsBothFound.isPresent()) {
            ...
            /*
             * this is also impossible case, taking into account the algorithm, but we keep
             * the code, for the same of completeness
             */
    ...
    public List<NamedEntityAnnotationImpl> getStoryOrItemAnnotation(String storyId, String itemId,
            String wikidataEntity) throws HttpException, IOException {
        String wikidataIdGenerated = null;
        wikidataIdGenerated = WikidataUtils.getWikidataEntityUri(wikidataEntity);
        return persistentNamedEntityAnnotationService.findAnnotations(storyId, itemId,
                EnrichmentConstants.MONGO_SKIP_FIELD, wikidataIdGenerated, EnrichmentConstants.MONGO_SKIP_LIST_FIELD);
    }
    ...
    private double computeScoreForAnnotations(List<String> foundByNer, List<String> linkedByNer) {
        ...
        if (!foundByNer.isEmpty()) {
            if (foundByNer.contains(NerTools.Dbpedia.getStringValue())
                    && foundByNer.contains(NerTools.Stanford.getStringValue())) {
                foundByDbpedia = 1;
                foundByStanford = 1;
        ...
        if (!linkedByNer.isEmpty()) {
            if (linkedByNer.contains(NerTools.Dbpedia.getStringValue())
                    && linkedByNer.contains(NerTools.Stanford.getStringValue())) {
                linkedByDbpedia = 1;
                linkedByStanford = 1;
